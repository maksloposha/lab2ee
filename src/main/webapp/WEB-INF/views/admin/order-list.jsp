        ...
        <div class="card">
            <c:choose>
                <c:when test="${empty orders}">
                    <div class="empty-table-msg">Замовлень у цій категорії немає.</div>
                </c:when>
                <c:otherwise>
                    <table>
                        <thead>
                            <tr><th>&#8470;</th><th>Клієнт</th><th>Адреса</th><th>Позицій</th><th>Сума</th><th>Статус</th><th>Дата</th><th>Дії</th></tr>
                        </thead>
                        <tbody>
                            <c:forEach var="order" items="${orders}">
                                <tr>
                                    ...
                                    <td class="fw-bold text-accent"><fmt:formatNumber value="${order.total}" pattern="#,##0.00"/>&nbsp;&#8372;</td>
                                    <td>
                                        <c:choose>
                                    ...
                                    <td class="text-muted cell-date"><food:formatDate value="${order.createdAt}" pattern="dd.MM.yyyy HH:mm"/></td>
                                    <td><a href="${pageContext.request.contextPath}/admin/orders/view?id=${order.id}" class="btn btn-outline btn-sm">Деталі</a></td>
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
